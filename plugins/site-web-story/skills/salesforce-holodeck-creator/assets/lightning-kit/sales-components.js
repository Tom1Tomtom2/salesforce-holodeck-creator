...
class LcSalesOverview extends JsonComponent {
  render() {
    ...
    this.innerHTML = `<section class="lc-sales-overview" aria-labelledby="sales-overview-title"><div class="lc-sales-overview__top"><div class="lc-sales-overview__identity"><span class="lc-object-icon lc-object-icon--forecast">${lcIcon('trend')}</span><div><span class="lc-sales-overview__eyebrow">${escapeHtml(data.eyebrow || 'Forecast')}</span><h1 id="sales-overview-title">${escapeHtml(data.title || 'Revenue Command Center')}</h1><p>${escapeHtml(data.subtitle || '')}</p></div></div><div class="lc-sales-overview__actions"><span class="lc-badge lc-badge--success">${escapeHtml(data.status || '')}</span>${actions}</div></div><div class="lc-sales-overview__metrics">${metrics}</div></section>`;
